...
CELL_STYLE   = 'style="padding:6px 10px;font-size:13px;color:#000;"'
...
def songs_to_html(songs: list, highlight: set = None) -> str:
    ...
    for idx, s in enumerate(songs):
        ...
        rows += (
            ...
            f'<td {CELL_STYLE}><b style="color:#000;">{s["title"]}</b></td>'
...
def remove_song_fn(song_choice, songs):
    """Removes a song from the playlist by index."""
    if not songs:
        return songs, songs_to_html(songs), "⚠️ No songs to remove."
    
    if not song_choice:
        return songs, songs_to_html(songs), "⚠️ Select a song to remove."
    
    # Extract index from the choice string (format: "1. Title - Artist")
    try:
        idx = int(song_choice.split(".")[0]) - 1
        if 0 <= idx < len(songs):
            removed_song = songs[idx]
            updated = songs[:idx] + songs[idx+1:]
            return updated, songs_to_html(updated), f"✅ '{removed_song['title']}' removed!"
        else:
            return songs, songs_to_html(songs), "⚠️ Invalid song selection."
    except (ValueError, IndexError):
        return songs, songs_to_html(songs), "⚠️ Error removing song."


def get_song_choices(songs):
    """Returns a formatted list of songs for the dropdown."""
    if not songs:
        return []
    return [f"{i+1}. {s['title']} - {s['artist']}" for i, s in enumerate(songs)]
...
with gr.Blocks(title="🎵 Playlist Vibe Builder") as demo:
    ...
    # ── Remove a song ──────────────────────────────────────────────────────────
    with gr.Accordion("🗑️ Remove a Song from the Playlist", open=False):
        gr.Markdown("Select a song and click **Remove** to delete it from your playlist.")
        with gr.Row():
            remove_dropdown = gr.Dropdown(
                choices=get_song_choices(DEFAULT_SONGS),
                label="Select a song to remove",
                interactive=True
            )
        remove_btn = gr.Button("🗑️ Remove Song", variant="stop")
        remove_status = gr.Markdown(value="")

    remove_btn.click(
        fn=remove_song_fn,
        inputs=[remove_dropdown, songs_state],
        outputs=[songs_state, playlist_display, remove_status],
    )

    # ── Update dropdown when songs change ──────────────────────────────────────
    def update_remove_dropdown(songs):
        """Updates the remove dropdown whenever songs_state changes."""
        return gr.Dropdown(choices=get_song_choices(songs))
